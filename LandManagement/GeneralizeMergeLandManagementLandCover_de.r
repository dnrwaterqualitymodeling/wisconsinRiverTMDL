...
if (!file.exists(paste(local_dir, basename(inCDL2MileBuffer), sep='/'))){
    ...
    anc = list.files(basename(inCDL2MileBuffer), bsnme)
    for (fl in anc){
        file.copy(paste(overlay_dir, fl, sep = '/'),
            paste(overlay_dir, fl, sep = '/'))
    }
...
readOGR(local_dir,
	substr(basename(inCDL2MileBuffer), 1, nchar(basename(inCDL2MileBuffer)) - 4),
...
writeRaster(
	raster(inLandCover),
	paste(local_dir, basename(inLandCover), sep="/"))
file.copy(paste(overlay_dir, remap1_txt, sep="/"), paste(local_dir, remap1_txt, sep="/"))
file.copy(paste(overlay_dir, remap2_txt, sep="/"), paste(local_dir, remap2_txt, sep="/"))
