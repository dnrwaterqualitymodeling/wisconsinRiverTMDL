...
for (rw in 1:length(unique(wrb_mukeys$MUKEY))){
#     rw <- 1
    agg_mky_tbl[] = 0
	mky <- wrb_mukeys$MUKEY[rw]
...
#     comp_data[,dat_cols] <- comp_data[,dat_cols] * comp_data[,'comppct_r']
    ...
    # making sure that zero texture values are set to NA
#     txtSums <- rowSums(comp_data[,c('sandtotal_r','silttotal_r','claytotal_r')])
#     grp_tbl@horizons[which(txtSums < 80),c('sandtotal_r','silttotal_r','claytotal_r')] <- NA
