...
for (hsg in LETTERS[1:4]) {
    ...
    names(clus_d_scld) = c(
        ...
        "cbn",
        "usle_k")
    clus_d_scld = melt(clus_d_scld, id.vars="hru_grp")
    clus_d_scld$hru_grp = as.factor(clus_d_scld$hru_grp)
    names(clus_d_scld)[3] = "z_score"
...
png(cluster_variability, 6.5, 9, "in", res=600)
grid.arrange(
    plot_list$A,
    plot_list$B,
    plot_list$C,
...
agg_soil_data[agg_soil_data$SNAM == "W", 
    c('MUID', 'CMPPCT', 'SOL_ALB1', "ANION_EXCL", "SOL_CRK")] <- c(001, 100, 0.08, 0.5, 0.5)
# givin' water a D
