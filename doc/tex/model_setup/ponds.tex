...
If the normal volumes were not listed in Wisconsin Lakes \cite{wisconsin_wisconsin_lakes_2009}, the maximum depth of the lake typically was listed. For those lakes where volume was not listed, we predicted volume based on a fit regression using maximum depth ($p < 0.001$) and surface area ($p < 0.001$) as predictors (Figure \ref{fig:volume_regressions}).

\begin{equation}
\bm{V} = e^{-0.1 + 1.1 \cdot \ln(\bm{A}) + 0.6 \cdot \ln(\bm{D})} 
\end{equation}

When maximum depth was not available, we fit a separate regression using only surface area. (Figure \ref{fig:volume_regressions}).
