...
#### CNOP has a huge influence, drowns everything else out
dat = subset(dat, !grepl("CNOP", parameter))
...
rch.dat = subset(dat, grepl("rte", dat$parameter))
sub.dat = subset(dat, !grepl("rte", dat$parameter))

for (var in unique(rch.dat$var_type)){
	indx = which(rch.dat$var_type == var)
	rch.dat$relative_rank[indx] = rch.dat$value[indx]/max(rch.dat$value[indx])
}

for (var in unique(sub.dat$var_type)){
	indx = which(sub.dat$var_type == var)
	sub.dat$relative_rank[indx] = sub.dat$value[indx]/max(sub.dat$value[indx])
}
...
### For parameters using subbasin output
png("~/sensitivity_figs/sensitivity_fig_sub.png",res=900,units='in',height=11,width=8)
# ggplt = ggplot(sub.dat, aes(x=reorder(parameter, value), y=value, fill = variable))
ggplt = ggplot(sub.dat, aes(x=reorder(parameter, relative_rank), y=relative_rank, fill = var_type))
ggplt = ggplt + geom_bar(stat="identity", position = "dodge") + coord_flip() + theme_bw()
ggplt = ggplt + facet_grid(var_type ~ season)
ggplt = ggplt + theme(
	axis.text.y=element_text(size=6, angle=0),
