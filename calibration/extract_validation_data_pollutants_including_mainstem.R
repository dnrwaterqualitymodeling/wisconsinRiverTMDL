...
dir_out = "T:/Projects/Wisconsin_River/GIS_Datasets/observed/usgs_loads/all/SS"
...
for (id in start_end_lu$station_id) {
	file_site = paste(dir_load, "/", id, ".gz", sep="")
	if (!file.exists(file_site)) { next }
	data_site = read.table(file_site, sep="\t", header=T)
	if (length(data_site$dload_00530) == 0) { next }
...
## PHOSPHORUS, MOST RECENT ESTIMATES FROM ROBERTSON AND HUBBARD
...
dir_out = "T:/Projects/Wisconsin_River/GIS_Datasets/observed/usgs_loads/all/TP"
val_dir = paste(dir_out, "/validation", sep="")
cal_dir = paste(dir_out, "/calibration", sep="")
#gage_subbasin_lu =
#	read.csv("T:/Projects/Wisconsin_River/GIS_Datasets/observed/gauge_basin_lookup.csv",
...
for (id in start_end_lu$station_id) {
	...
	if (!file.exists(file_site)) { next }
	data_site = read.table(file_site, sep="\t", header=T)
	if (length(data_site$dload_00665) == 0) { next }
	data_site$date = as.Date(data_site$date, format="%m/%d/%Y")
	start_year = format(subset(start_end_lu, station_id == id)$start, "%Y")
	end_year = format(subset(start_end_lu, station_id == id)$end, "%Y")
	data_site = subset(
...
dir_load = "T:/Projects/Wisconsin_River/GIS_Datasets/observed/usgs_loads/USGS_NWIS"
dir_out = "T:/Projects/Wisconsin_River/GIS_Datasets/observed/usgs_loads/all/TP"
val_dir = paste(dir_out, "/validation", sep="")
cal_dir = paste(dir_out, "/calibration", sep="")
gage_subbasin_lu =
	read.csv("T:/Projects/Wisconsin_River/GIS_Datasets/observed/gauge_basin_lookup.csv",
		colClasses=c(rep("character", 4), "integer", "integer", "character"))
fraction_for_validation = 1/4
full_yr_threshold = round(365 / 2)

set.seed(2558698)

usgs_ids = c("5390680", "5390685", "5392070", "5392083", "5392100")
gage_subbasin_lu = subset(gage_subbasin_lu, LOAD_ID %in% usgs_ids)

summ_table = NULL
for (id in gage_subbasin_lu$LOAD_ID) {
	file_site = paste(dir_load, "/0", id, ".txt", sep="")
	data_site = read.table(file_site)
	data_site = data_site[data_site[,5] == "A",]
