...
file_plots = "T:/Projects/Wisconsin_River/Model_Outputs/plots/USLE_P_plots/median_tp_may_oct/usle_p_scens_wo_pt_srcs.pdf"
...
scens = tbl(con, sql("SELECT DISTINCT usle_p_SCEN FROM usle_p_scenarios")) %>%
	collect()
scens = scens[[1]]
rm(con)
...
for (scen in scens) {
	con = src_sqlite(file_db)
	scen_d = tbl(con, "USLE_P_scenarios") %>%
		filter(USLE_P_SCEN == scen) %>%
	...
	ttl = paste(-scen*100, "% reduction of calibrated USLE_P value", sep="")
