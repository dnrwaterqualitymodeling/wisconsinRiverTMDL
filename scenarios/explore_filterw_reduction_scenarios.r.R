...
for (scen in scens) {
	...
	for (file_mgt in files_mgt) {
		...
		if (ag) {
			filterw = as.numeric(str_sub(mgt[14], 7, 16))
			filterw = filterw * (1 + scen)
			filterw = sprintf("%10.6f", filterw)
			str_sub(mgt[14], 7, 16) = filterw
		}
	...
	output_scen = tbl(con, "output_rch_monthly_ag_scen_no_point_src_ms4") %>%
		...
		mutate(FILTERW_SCEN = scen) %>%
		select(RCH, FILTERW_SCEN, FWMC) %>%
		collect()
...
output_scens = copy_to(con, output_scens, "FILTERW_scenarios", temporary=F)
